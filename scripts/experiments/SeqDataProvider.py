...
class SeqDataProvider(DataProvider):
    ...
    def __init__(self, data_path, which_set='train', segment_size=150000, element_size=1000, feature_size=58, overlap=False,
        batch_size=1, rng=None, partial=False):
        ...
        if not partial:
            ...
            signal = np.loadtxt(file_path_x, delimiter=",", skiprows=1, max_rows=1500000)
            time = np.loadtxt(file_path_y, delimiter=",", skiprows=1, max_rows=1500000)
